...
export const FormLabel = (props) => {
    ...
    return renderLabel ? renderLabel(props) :
        <label className={props.className} htmlFor={field_def.secure}>{field_def.label}</label>
};

export const FormTextInput = (props) => {
    const {def, renderInput, autoComplete, form, validateField, values, isSubmitting, setFieldValue, name} = props;
    const field_def = props.field_def || def[name];
    ...
    return renderInput ? renderInput(props) :
...
export const FormSubmitButton = (props) => {
    ...
    return (
        renderButton ? renderButton(props) :
            <button
                className={`button is-primary is-medium is-fullwidth form-button-submit ${isSubmitting ? "is-loading" : ""}`}
                inactive={!isValid || isSubmitting}
                type={"submit"}>{button_label}{isSubmitting ?
                <span className={"form-button-submit-spinner"}/> : false}</button>
    )
};
        ...
        >
            {props => {
                ...
                return (
                    ...
                    <form onReset={props.handleReset} className="form" onSubmit={props.handleSubmit}
